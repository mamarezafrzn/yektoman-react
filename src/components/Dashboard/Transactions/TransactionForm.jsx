...
const TransactionForm = (props) => {
  ...
  const [price,setPrice] = useState({value:"", validation: { isValid: true },})
  const [report_code,setReport_code] = useState({value:"", validation: { isValid: true },})
  const [pay_for_other,setPay_for_other] = useState()

  const onDateChange = (value) => {
    setDateValue({value:value.format("YYYY/MM/DD"), validation: { isValid: true },});
  };
  const onFundSelect = (event) => {
    setFund({value:event.target.value, validation: { isValid: true },});
  };
  const onPriceChange = (event) => {
    setPrice({value:event.target.value, validation: { isValid: true },});
  };
  const onReportCodeChange = (event) => {
    setReport_code({value:event.target.value, validation: { isValid: true },});
  };
  ...
  const onFormSubmit = (event) =>{
    ...
    setDateValue({...dateValue, validation: fund.value.length <= 0 ? {isValid:false , errorMsg:"این فیلد ضروری است!"}: {isValid:true}})
    setPrice({
        ...price,
        validation: checkIfNumber(price.value),
      });
      setReport_code({...report_code, validation: checkIfNumber(report_code.value)})

    if(fund.validation.isValid && dateValue.validation.isValid && price.validation.isValid && report_code.validation.isValid ){
        createTransaction({
            fund_id : fund.value,
            price:price.value,
  ...
  return (
    <form className={styles.newForm} onSubmit={onFormSubmit}>
        ...
        >
          <FormControl fullWidth>
            ...
            <Select
              ...
              label="Age"
            >
              {props.funds.length >= 1 && props.funds?.map((item) => 
      ...
      <label>
        کد رهگیری
        <input type="text" value={report_code.value} onChange={onReportCodeChange}/>
      </label>
      <label>
      پرداخت برای دیگران
        <input type="text" placeholder="کد ملی" value={pay_for_other} onChange={onPayForOthersChange}/>
      </label>
